...
# pytorch implementation of Temporal Fusion Transformer 
# https://github.com/PlaytikaOSS/tft-torch


class DatasetTFT(Dataset):

    # build datset for TFT implementation
...
def collate_fn_TFT(batch):
    ...
    feature_x_tensor = feature_x_tensor.reshape((batch_size,past_window,feature_dim)) # batch_size * past_window * feature_dim
    x_tensor = torch.cat([resid_x_tensor, feature_x_tensor], axis=-1) # batch_size * past_window * (feature_dim+1)

    featureresid_y_tensor = featureresid_y_tensor.reshape((batch_size, num_step_prediction, 1)) # batch_size * num_step_prediction * 1
    feature_y_tensor = feature_y_tensor.reshape((batch_size,num_step_prediction,feature_dim)) # batch_size * num_step_prediction * feature_dim
    ...
    # we do not consider static feature (assinged 1-dim zero)
    return {'static_feats_numeric' : torch.zeros((batch_size,1)), 'historical_ts_numeric' : x_tensor, 'future_ts_numeric' : y_tensor}, resid_y_tensor

def make_batch_TFT(single_residX, single_featureX, single_featureY):
    
    # single_residX : window_len
    # single_featureX : feature_dim * window_len
    # single_featureY : feature_dim

    batch_size = 1 # make batch with single prediction point
    feature_dim, window_len = single_featureX.shape[0], single_featureX.shape[1]
    resid_x_tensor = torch.Tensor(single_residX).reshape((batch_size,window_len,1))
    feature_x_tensor = torch.Tensor(single_featureX).reshape((batch_size,window_len,feature_dim))
    x_tensor = torch.cat([resid_x_tensor, feature_x_tensor], axis=-1) # batch_size * window_len * (feature_dim+1)
    feature_y_tensor = torch.Tensor(single_featureY).reshape((batch_size,1,feature_dim))
    
    single_batch = {'static_feats_numeric' : torch.zeros((batch_size,1)), 
                    'historical_ts_numeric' : x_tensor, 'future_ts_numeric' : feature_y_tensor}
