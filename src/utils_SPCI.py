...
def get_features_strided(feature_matrix, L, S):

    features_strided_list = []

    for i in range(feature_matrix.shape[-1]):
        feature_arr = feature_matrix[:,i]
        features_strided = strided_app(feature_arr, L, S)
        features_strided_list.append(features_strided)

    return np.stack(features_strided_list, -1) # sample_size * seq_len * feature_dim
...
def binning_use_RF_quantile_regr(quantile_regr, Xtrain, Ytrain, feature, beta_ls, sample_weight=None):
    ...
    low_high_pred = quantile_regr.fit(Xtrain, Ytrain,sample_weight).predict(feature) # num of predictions = len(full_alphas)
